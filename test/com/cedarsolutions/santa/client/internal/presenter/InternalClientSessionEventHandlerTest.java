...
/**
 * Unit tests for InternalClientSessionEventHandler.
 * @author Kenneth J. Pronovici <pronovic@ieee.org>
 */
public class InternalClientSessionEventHandlerTest extends StubbedClientTestCase {

    /** Test getModule(). */
    @Test public void testGetModule() {
        InternalClientSessionEventHandler handler = createEventHandler();
        assertEquals(Module.INTERNAL, handler.getModule());
    }

    /** Create a properly-mocked handler, including everything that needs to be injected. */
    private static InternalClientSessionEventHandler createEventHandler() {
        ...
        InternalClientSessionEventHandler handler = new InternalClientSessionEventHandler();
